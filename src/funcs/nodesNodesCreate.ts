 ...
 */
export async function nodesNodesCreate(
  client: AtomaSDKCore,
  request: components.NodesCreateRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.NodesCreateResponse,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => components.NodesCreateRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/nodes")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.NodesCreateResponse$inboundSchema),
    M.fail(["4XX", 500, "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
